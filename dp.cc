...
std::vector< ONE_VIDEO_PARAMS > get_all_video_params(std::string path) {
	/*
		New function : taken in the path to folder wherein we have 
		text files as video descriptors, parses each file iteratively
		and populates ONE_VIDEO_PARAMS structure for each video
	*/
	...
	if (dir != NULL) {
		while ((entry = readdir(dir)) != NULL) {
			...
			if (in_file.is_open())
			{
				...
				if(! getline(in_file, line)) continue;
				...
				assert(chunkno == cur_video.TOTAL_VIDEO_CHUNCK);
